...
if __name__ == "__main__":
    ...
    #parser.add_argument("--input_dir", type=str, default="/raid/brutusxu/function_calls/logs/OnlyFunctionNames")
    ...
    print("Shared_pattern_dict\n")
    print(shared_pattern_dict);

    function_calls = function_calls_dict[args.fname_to_remove][0]
    print("Removing patterns from: ", args.fname_to_remove)
    print(f"\nlength before filtering -> {len(function_calls)}")

    for k, v in shared_pattern_dict.items():
        print(f"\ncurrent target pattern -> {k}")
        function_calls = remove_pattern_greedy(k, function_calls)

    print(f"\nlength after filtering -> {len(function_calls)}")
    for functions in function_calls:
