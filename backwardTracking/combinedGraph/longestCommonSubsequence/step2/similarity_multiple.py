...
#             print('j', j)
#             subsequence_a = list_a[i:j]
#             for k in range(len(list_b)):
#                 for l in range(k + 1, len(list_b) + 1):
#                     subsequence_b = list_b[k:l]
#                     if subsequence_a == subsequence_b and len(subsequence_a) > len(longest_common_subsequences):
#                         longest_common_subsequences = subsequence_a
#         length = len(longest_common_subsequences)
#         if length > 0:
#             res.append(longest_common_subsequences)
#             i += length
#         else:
#             i += 1
#     return res
...
def longest_common_line_sequence(directory: str, pattern: bool = False, order: bool = False, filename: str = None) -> list:
    EMPTYLINE = ''
