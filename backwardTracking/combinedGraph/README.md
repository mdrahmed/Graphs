...
```
./combined-graph-v3.py ##This file is a copy of ./combined-graph-v2.py, I used this to draw the **motivation/attack** part of the paper. 
```
...
### Retrieve only function names
Following program will get only the function names,
```
functionNames.py
```


### Extract Patterns Generated by Plagarism Finder
I have used this file to find the common text between `hbw` and `vgr`,
```
/home/raihan/Graphs-all/Graphs/backwardTracking/combinedGraph/plagarismFinder/myPlagiarismChecker/2plag.py
```
Saved the output in `output2` with this `python 2plag.py > output2`.

**commons.sh** inside 
Then I used the `commons.sh` script to remove the common patterns between `hbwall3.txt` and `vgrall3.txt` file. The `commons.sh` will run `removeCommons.py` and save the output inside the `patterns/patterns.txt` file.


**scripts/find_graphs_of_Longest_Pattern.sh** inside `combinedGraph` folder.
From the `plagarismFinder/myPlagiarismChecker/patterns/patterns.txt`, I found the longest pattern and got the function names from it and then I used the `scripts/find_graphs_of_Longest_Pattern.sh` script to extract the graphs that will contain those function name strings on them and the output will be saved inside the `longestPatternRemoved.txt`.

**Run it a couple of times and check the 1st graph. That often contains the function names in order as I used `rg`**
