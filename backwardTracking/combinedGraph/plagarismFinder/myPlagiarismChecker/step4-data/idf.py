...
if __name__ == "__main__":
    ...
    parser.add_argument("--input_dir", type=str, default="../step3-data/uniqueTraces")
    ...
    fout_name = f"mostFrequentFunctions{last_part}" 
    ...
    idfTrace_name = f"idf_updated_traces_{freq}_{last_part}"
    ...
    with open(input_file, "r") as fin, open(idfTrace_file, "w") as fout:
        for line in fin:
            if line.strip().lower() not in filtered_words: 
                fout.write(line)
        print(f"Unique variant traces updated by removing the functions greater than {freq}")
