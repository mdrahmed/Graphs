...
def parse_input(input_str, topic):
    ...
    for line in reversed(input_str.split('\n')):
        ...
        if "Global" in line and found:
            if line.split()[-1] == '1':
                ...
                if topic is not None:
                    current_graph.edge(global_var, topic, dir="back")
                    topic = None
                    i += 1
                    edges += 1
            ...
            if len(globals) > 0:
                ...
                edges += 1
            if topic is not None:
                current_graph.edge(function_name, topic, dir="back")
                topic = None
                i += 1
                edges += 1
            ...
            if len(globals) > 0:
                ...
                edges += 1
            if topic is not None:
                current_graph.edge(callee, topic, dir="back")
                topic = None
                i += 1
                edges += 1
        ...
        #prev_line = line
        ## Need to check if there is multiple topics
        elif "topic:" in line and found:
            topic = line
            current_graph.node(topic)
            if len(globals) > 0:
                #current_graph.edge(globals[-1], function_name)
                current_graph.edge(topic, globals[-1], dir="back")
                globals = []
                i += 1
                edges += 1
            elif len(callInsts) > 0:
                #current_graph.edge(callInsts[-1], function_name)
                current_graph.edge(topic, callInsts[-1], dir="back")
                callInsts = []
                i += 1
                edges += 1
            elif len(functions) > 0:
                #current_graph.edge(functions[-1], function_name)
                current_graph.edge(topic, functions[-1], dir="back")
                i += 1
                edges += 1
            #functions.append(function_name)
            if 'vgr' in topic:
                ...
                found = False
                graphs2 = parse_input(vgr_str, topic)
                found = True
            graphs.append(graphs2, topic)
...
graphs = parse_input(input_str, None)
